...
napi_value reporter::ctor(napi_env env, napi_callback_info info) {
    ...
    // endpoint
    {
        ...
        const auto &server_address = std::string(buf, buf_len);
        set_env_var("GREENER_INGRESS_ENDPOINT", server_address);
    }
    ...
    // api key
    {
        ...
        const auto &api_token = std::string(buf, buf_len);
        set_env_var("GREENER_INGRESS_API_KEY", api_token);
    }
    ...
    const greener_reporter_error *err;
    obj->reporter_ = greener_reporter_new(&err);
    if (err != nullptr) {
...
napi_value reporter::create_session(napi_env env, napi_callback_info info) {
    ...
    // id
    {
        ...
        if (!(in_type == napi_string || in_type == napi_null)) {
            napi_throw_error(env, nullptr,
                             "session_id must be a nullable string");
            return nullptr;
        }
    ...
    // description
    {
        ...
        if (!(in_type == napi_string || in_type == napi_null)) {
            napi_throw_error(env, nullptr,
                             "description must be a nullable string");
            return nullptr;
        }
    ...
    if (in_session_id.has_value()) {
        set_env_var("GREENER_SESSION_ID", in_session_id.value());
    }
    if (in_description.has_value()) {
        set_env_var("GREENER_SESSION_DESCRIPTION", in_description.value());
    }
    if (in_baggage.has_value()) {
        set_env_var("GREENER_SESSION_BAGGAGE", in_baggage.value());
    }
    if (in_labels.has_value()) {
        set_env_var("GREENER_SESSION_LABELS", in_labels.value());
    }
    ...
    std::unique_ptr<const greener_reporter_session, decltype(session_deleter)>
        session(greener_reporter_session_create(obj->reporter_, &err),
                session_deleter);

    if (in_session_id.has_value()) {
        unset_env_var("GREENER_SESSION_ID");
    }
    if (in_description.has_value()) {
        unset_env_var("GREENER_SESSION_DESCRIPTION");
    }
    if (in_baggage.has_value()) {
        unset_env_var("GREENER_SESSION_BAGGAGE");
    }
    if (in_labels.has_value()) {
        unset_env_var("GREENER_SESSION_LABELS");
    }
    ...
    if (const auto status = napi_set_named_property(env, out_session_value,
                                                    "id", out_session_id_value);
        status != napi_ok) {
        handle_napi_error(env);
        return nullptr;
    }
...
napi_value reporter::create_testcase(napi_env env, napi_callback_info info) {
    ...
    // testcase_file
    {
        ...
        if (!(value_type == napi_string || value_type == napi_null)) {
            napi_throw_error(env, nullptr,
                             "testcase_file must be a nullable string");
            return nullptr;
        }
    ...
    // testsuite
    {
        ...
        if (!(value_type == napi_string || value_type == napi_null)) {
            napi_throw_error(env, nullptr,
                             "testsuite must be a nullable string");
            return nullptr;
        }
    ...
    // status
    {
        ...
        if (in_status != "pass" && in_status != "fail" && in_status != "err" &&
            in_status != "skip") {
            napi_throw_error(env, nullptr,
    ...
    // output
    {
        ...
        if (!(value_type == napi_string || value_type == napi_null)) {
            napi_throw_error(env, nullptr,
                             "stdout_stream must be a nullable string");
            return nullptr;
        }
    ...
    greener_reporter_testcase_create(
        ...
        in_testcase_classname.has_value()
            ? in_testcase_classname.value().c_str()
            : nullptr,
        in_testcase_file.has_value() ? in_testcase_file.value().c_str()
                                     : nullptr,
